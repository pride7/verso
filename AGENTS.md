...
| **M5b 远端同步**（v0.6.38–40 攒成） | **`v0.7.0`** ✅ |
| ↳ 修：同步改写打开中的笔记后不再弹「外部修改」横幅（§2.8） | `v0.7.1` ✅ |
| ↳ 移动端适配第一批：后台恢复对账、触摸入口快修、树的免拖拽移动/排序（M6，§1.2/§2.7） | `v0.7.2` ✅ |
| ↳ 修：空表 / 窄表的数据库设置面板不再被裁掉（§2.6） | `v0.7.3` ✅ |
| ↳ 空白笔记可直接添加第一条 frontmatter 属性（§2.3） | `v0.7.4` ✅ |
| ↳ AI 协作：终端可靠右竖排、一个键把笔记/选区发给终端（§7.3/§7.6） | `v0.7.7` ✅ |
| ↳ 修：公式块不混进目录，渲染占位收紧（§4.2/§6.3） | `v0.7.8` ✅ |
| ↳ 为 vault 自动生成 AI CLI 仓库说明（§7.7） | `v0.7.9` ✅ |
| ↳ 修：打开终端时不再恢复 AGENTS.md（§7.7） | `v0.7.10` ✅ |
| ↳ 多人协作第一步：加入共享仓库、协作动态与未读变化（§2.8） | `v0.7.11` ✅ |
| ↳ 按内容节点共享：子树与附件迁入按成员组复用的空间，GitHub 自动建库邀请（§2.8） | `v0.7.12` ✅ |
| ↳ 思维导图：触摸化（单指拖 / 双指捏 / 节点菜单）+ 文字换行按实测高度排版 + 宽度可拖 + 唯一一级标题当根（M6，§4.7） | `v0.7.13` ✅ |
| ↳ 共享空间管理：实时成员、继续邀请、移回私人（§2.8） | `v0.7.14` ✅ |
| ↳ 语义冲突：frontmatter / database 按字段合并，正文可两边保留（§2.8） | `v0.7.15` ✅ |
| ↳ 修改建议：共享空间逐文件审阅，结论随正式版本同步（§2.8） | `v0.7.16` ✅ |
| ↳ 科研项目工作台：一屏现状 + 分类快速记录 + 普通 Markdown 子文档（§2.10） | `v0.7.17` ✅ |
| ↳ 项目研究文档：分类模板 + 创建后进入完整编辑器（§2.10） | `v0.7.18` ✅ |
| ↳ 项目中心：跨项目汇总 + 单项目总览打磨 + 右键菜单边缘避让（§2.10/§6.3） | `v0.7.19` ✅ |
| ↳ 修：项目摘要不重复进正文 + 状态单选统一 + 视图直属范围与筛选可读化（§2.3/§2.6/§2.10） | `v0.7.20` ✅ |
| ↳ 移动端可用性大修：底部导航、触摸尺寸令牌、公式条按需出现（M6，§1.2/§5.5/§6.3） | `v0.7.21` ✅ |
| ↳ 官网首页与 README 重构：产品信息架构、项目中心展示与中文说明优化 | `v0.7.22` ✅ |
| ↳ 终端最大化编辑区 + 活动记录时间线收敛（§2.8/§7.3） | `v0.7.23` ✅ |
| ↳ 移动端：键盘弹起来时底部导航/状态栏让位 + 公式条收成一行（M6，§5.5） | `v0.7.26` ✅ |
| ↳ 项目分类可增删排序，写在项目笔记的 `sections` 里（§2.10） | `v0.7.27` ✅ |
| ↳ 修：动态面板一直没有默认快捷键（`Mod+Shift+H`，§6.3） | `v0.7.28` ✅ |
| ↳ 看板 / 日历的免拖拽等价物 + 窄屏议程 + 三处 `window.prompt` 换成自绘输入框（M6，§2.6） | `v0.7.29` ✅ |
| ↳ 点击目标审计扩到十五屏，修大纲 / 动态 / 日历工具条 / 加一列（M6，§1.2） | `v0.7.30` ✅ |
| ↳ 正文里的右键菜单：文本格式 / 段落设置 / 插入三个二级菜单 + 剪贴板 / 表格（§4.10） | `v0.7.31` ✅ |
| ↳ 改动可一处一处退回 + 自动记录版本的总开关 + 文档树一键折叠（§2.8/§2.1） | `v0.7.32` ✅ |
| ↳ 快速创建共享空间时可自己起名（建好就改不了，必须建之前问，§2.8） | `v0.7.33` ✅ |
| ↳ 状态四档语义色 + 可删除 + 总览里就地改名（§2.10/§6.2） | `v0.7.34` ✅ |
| ↳ 记录可置顶，可在分类之间移动（§2.10） | `v0.7.35` ✅ |
| ↳ 修：状态菜单里的勾和叉共用行尾一格，不再差半格 | `v0.7.36` ✅ |
| ↳ 时间列可筛选 + 相对时间 `90d ago` + `/` 知识库骨架（§2.6） | `v0.7.37` ✅ |
| ↳ 知识库视图文案规范化 | `v0.7.38` ✅ |
| ↳ 修输入法顶掉公式；视图块边界不算进入；表格内改名；树里收起一支（§4.2/§2.6/§2.1/§2.10） | `v0.7.39` ✅ |
| ↳ 状态统一成三档：未开始 / 进行中 / 已完成（§2.10） | `v0.7.40` ✅ |
| ↳ 修：视图工具条上多一个按钮就把齿轮挤到中间（flex 的 auto 会平分空间） | `v0.7.41` ✅ |
| ↳ 文案规范化：视图源码入口与文档树菜单 | `v0.7.42` ✅ |
| ↳ 模板目录不在文档树里重复显示（§4.6） | `v0.7.43` ✅ |
| ↳ 修：冲突面板反复弹，选哪边都同步不完（§2.8） | `v0.7.44` ✅ |
...
| ↳ 打印 / 导出 PDF（**原属 M8+，提前**；§2.12） | `v0.7.49` ✅ |
| ↳ 打开草稿箱直接进卡片界面，按 `type: scratch` 判（§4.7.1） | `v0.7.50` ✅ |
...
| ↳ 纯文件夹可删除 / 建子文档 / 创建为文档（§2.1） | `v0.6.30` ✅ |
| ↳ 修：仓库路径显示 `\\?\` 前缀（Windows） | `v0.6.31` ✅ |
| ↳ 提交署名可在同步设置中配置（§2.8） | `v0.6.32` ✅ |
| ↳ 模板面板可直接新建，并补全变量说明（§4.6） | `v0.6.33` ✅ |
| ↳ 模板可打开、插入、改名和删除（§4.6） | `v0.6.34` ✅ |
...
| ↳ 同步报错人话化（§2.8） | `v0.6.39` ✅ |
| ↳ 冲突解决面板：逐段选边 + 定稿重放；修 autocrlf 改写行尾（§2.8） | `v0.6.40` ✅ |
| **M6a 移动端（安卓）** | `v0.8.0` ✅ |
| ↳ 修：思维导图可返回正文，项目标签复用，记录可删除且新建默认进行中 | `v0.8.1` ✅ |
...
| ↳ 草稿台更名收集箱并生成直属子文档；正文算式可预览并写入结果（§4.7.1/§4.3） | `v0.8.6` ✅ |
| ↳ 网页粘贴转 Markdown；文件拖入归档；附件缺失定位与未使用清理（§4.3/§4.4） | `v0.8.7` ✅ |
| ↳ 项目标签按项目复用：同项目内容互相替换，跨项目并排保留（§2.1/§2.10） | `v0.8.8` ✅ |
| ↳ 修：不同项目的首个内容标签按打开顺序追加，不再插到前面（§2.1） | `v0.8.9` ✅ |
| ↳ 修：database 视图改名不再依赖可见的文档树节点（§2.1/§2.6） | `v0.8.10` ✅ |
| ↳ 修：HTML 粘贴只转义真正的强调下划线，变量名与公式下标保持原样（§4.3） | `v0.8.11` ✅ |
| ↳ 修：HTML 粘贴保留 `\[...\]` 定界符，块公式可继续转成 `$$...$$`（§4.3） | `v0.8.12` ✅ |
| ↳ 修：HTML 粘贴只转义真正的强调星号与行首列表符号（§4.3） | `v0.8.13` ✅ |
| ↳ 打字时提示还差几个字符的 snippet；括号不再自动套 `\left` `\right`（§5.3 / §5.4） | `v0.8.15` ✅ |
| ↳ 修：只有样式没有结构的 HTML 按纯文本粘贴，复制来的 `**加粗**` 不再被转义（§4.3） | `v0.8.16` ✅ |
| ↳ 侧栏大纲每一级标题都能收起（§6.3） | `v0.8.17` ✅ |
| ↳ 项目总览的每条记录显示创建日期（§2.10） | `v0.8.18` ✅ |
| ↳ 修：转义只在 HTML 真会产出 Markdown 标记时才做，多段源码粘贴不再被改写（§4.3） | `v0.8.19` ✅ |
| ↳ 大纲的「全部折叠 / 全部展开」，与文档树同一个位置（§6.3） | `v0.8.20` ✅ |
| ↳ 修：复制来的 Markdown 源码保持原来的行距，挨着的行不再被隔开空行（§4.3） | `v0.8.21` ✅ |
| ↳ Mermaid 图：块内渲染 + 编辑时预览 + 导出打印（§4.11） | `v0.8.22` ✅ |
| ↳ 修：图表的「编辑」按钮改成常显 + 触摸尺寸，手机上不再等于没有（§4.11/§5.5） | `v0.8.23` ✅ |
| ↳ 终端改成用到才加载，启动包 1.85 → 1.51 MB（§3.1） | `v0.8.24` ✅ |
| ↳ 修：Windows 上复制来的内容（CRLF）粘不进去，光标越界导致整次粘贴静默作废（§4.3） | `v0.8.25` ✅ |
| ↳ 修：思维导图跟着人跑到每一篇笔记上，换页不再回正文（§4.7） | `v0.8.26` ✅ |
| ↳ 修：项目分类名带「记录 / 文档」时，界面文字拼成「一条记录记录」（§2.10） | `v0.8.27` ✅ |
| ↳ 修：设置图标画的是太阳（放射线）而不是齿轮，重画成 6 齿（§6） | `v0.8.28` ✅ |
| ↳ 粘贴的块公式排成独占几行；和正文挤在一行时后半句也照常渲染（§4.3） | `v0.8.29` ✅ |
| ↳ 修：打印印的是打开那一刻的正文；新增导出长图（PNG / JPG）与复制到剪贴板（§2.12） | `v0.8.30` ✅ |
| ↳ 修：这一页显示的是另一篇的思维导图 —— 换页读盘的空档与后到的旧结果（§4.7/§2.1） | `v0.8.31` ✅ |
| ↳ 修：表格格子里全选再删，删掉的是正文里的字（§4.9） | `v0.8.32` ✅ |
| ↳ 项目中心：置顶 + 排序方式可选（§2.10） | `v0.8.33` ✅ |
| ↳ 修：笔记文件夹直接放在盘符下面（`D:\笔记`）时打不开（§2.1） | `v0.8.34` ✅ |
| ↳ 加入共享空间：粘贴邀请即可，位置与身份自动填好（§2.8） | `v0.8.35` ✅ |
| ↳ 修：置顶组按置顶先后排，先钉的在前（§2.10） | `v0.8.36` ✅ |
| ↳ 正文里的 `[文字](地址)` 渲染成链接，点一下走系统浏览器（§4.2） | `v0.8.37` ✅ |
| ↳ 修：改名 / 新建之后 database 视图不刷新（信号没发 + widget 里那棵树收不到，§2.6） | `v0.8.38` ✅ |
| ↳ 修一批：公式触发词被短的挡死、自定义 snippet 覆盖不了内置、日历默认空白、多处静默失败（§5.4/§2.6/§2.7/§4.7/§4.11） | `v0.8.39` ✅ |
| ↳ 修一批会丢内容的：写盘期间的键入被标成已保存、外部修改横幅拦不住保存、属性写入回读覆盖正文、frontmatter 解析失败被删、重复 id 炸索引；以及导出链接的协议白名单能被控制字符绕过（§2.7/§2.3/§2.5/§7.5） | `v0.8.40` ✅ |
...
| ↳ 修：Tab 缩进了，光标却留在原地（§5.1） | `v0.8.45` ✅ |
| ↳ 修：等宽字体的连字把代码块里的 `<=` 画成 `≤`（§6.1） | `v0.8.46` ✅ |
| ↳ 修：看板列多了被窗口切掉 —— 视图撑宽了正文（§2.6） | `v0.8.47` ✅ |
...
3. 同步检查并更新 [README.md](README.md)：至少核对当前版本、项目现状、平台支持、下载说明和路线状态，不能让 README 停在旧版本。
4. 提交，然后打 tag：`git tag v0.2.0`
...
`host/clipboard.ts` 是唯一的入口。写（`copyText`）在 `navigator.clipboard`
失败时退回 `execCommand("copy")`；读（`readText`）**没有兜底** ——
`execCommand("paste")` 在 Chromium 系里被明令禁掉了，网页能读剪贴板等于能偷走
用户刚复制的密码。所以读失败时必须说出还有哪条路（`Ctrl/⌘+V` 走的是浏览器
自己的通道，不受影响），静默失败的表现是「点了没反应」。

### 要一句输入时：`useAsk()`，不是 `window.prompt`

**WebView2 压根不实现 `prompt`**（Windows 上 Tauri 用的就是它），安卓 WebView
上也可能被吞掉 —— 两种情况都返回 `null`，也就是「用户按了取消」：按钮点下去
什么都不发生，还不报错。这三处曾经因此一直是死的：记版本说明、提交修改建议、
数据库列改名。

浏览器测试查不出来（Playwright 的 Chromium 有原生 prompt，怎么写都绿），所以
`tests/unit/app/noGlobalDialog.test.ts` 扫源码把它和 `window.confirm` 一起禁了。
换成 `ui/AskDialog.tsx` 的 `useAsk()`：`const { ask, askUI } = useAsk()`，
**记得把 `askUI` 挂进 JSX**，否则框永远不出现。

做成 hook 而不是全局 context 是有原因的：database 视图长在 CodeMirror widget
里，是另一棵 React 树，够不到 App 的 provider。
...
`tests/browser/app/mobile.test.tsx` 的「手机上的点击目标」那一组，现在覆盖
**十五个屏**：抽屉 · 正文 · 右键菜单 · 命令面板 · 设置 · 侧栏六个面板 ·
属性条 · database 的表格/看板/日历 · 思维导图 · 项目中心。

**加新界面时把它对应的屏也加进去**（照着写：把那一屏摆到 390×844 上，
`expect(tooSmall()).toEqual([])`）。清单靠人写必然有漏 —— 看板和日历的触摸
入口是翻旧清单才想起来的，而大纲行 27px、动态里的文件行 21px、日历的翻月键
25×29 这三处从来没人提过，是这组测试扫出来的。

三条判定规则写在测试里，别绕过：**行内的次要图标按 32 算**（撑到 44 会把
那一行整个顶开，`SECONDARY` 列表）、**中心点点不到的跳过**（它被浮层盖着，
本来就不该点得到）、**骑在屏幕边上或滚出容器的跳过**（滑一下就整个进来，
不是尺寸问题）。

顺带一条踩过的：**同一条规则里写两行 `min-height` 时，后一行会把前一行整个
盖掉**。`.outline-row` 曾经是 `min-height: var(--tap)` 紧跟 `min-height: 27px`,
于是触摸令牌在那里从来没生效过。要两者取大只有一种写法：`max(27px, var(--tap))`。
...
### 定尺寸的图标按钮：`padding: 0` 必须自己写

全局的 `button` 重置（styles.css 顶上那段）只清了 font / color / background /
border / cursor，**没有清 padding** —— 浏览器默认那份是 `1px 6px`。

于是把一个 `<span>` 改成 `<button>` 时会踩到：一个 13×13、`box-sizing:
border-box` 的勾选框，减掉 3px 边框再减掉 12px 内边距，内容盒是负数，里面那个
图标被压成**零宽**。症状极难认 —— DOM 里图标在、类名对、颜色对，
...
**browser 测试里不要靠「睡够 N 毫秒」等异步结果。** 整套并行跑起来的时候
机器是忙的，单跑时够用的那个数字就不够了；而且 **vitest 按文件大小决定并发
顺序**，所以**改动一个毫不相干的测试文件的长度，就可能让另一个文件换个结果**
—— 已经踩过一次：给 `tests/browser/editor/indent.test.ts` 加了三行断言，
`tests/browser/app/reorder.test.tsx` 开始整片失败，报的是「树里没有 X，实际有：」
后面空空如也。这种失败最容易被误判成「我刚才的改动搞坏了别处」，而真相是
那边一直在赌时序。等到**你要的东西真的出现**为止（轮询 + 一个说人话的超时
断言），别睡固定时长。
...
CM6 把选区映射过一次改动时用的是 `assoc = -1`：插在光标那个位置的文本会落在
光标**右边**，光标留在原地。行首插缩进、光标又正好在行首时，屏幕上的效果是
**「按了 Tab 什么都没发生」** —— 字确实缩进了，可光标没动，得用鼠标点一下
它才回到该在的地方。Tab 缩进（`editor/indent.ts`）就是这么栽的。

事务里自己指定落点：整行移动用 `state.selection.map(changeSet, 1)`（CM 自己的
`indentMore` 就是这么写的），在光标处插一段文本用 `selection: { anchor: from +
insert.length }`。**光标落点要写进测试** —— 只断言文档内容的话，这个 bug 一个
字都不会漏出来。
...
### 宽 widget 会把整个正文撑宽，`overflow-x: auto` 救不了它

v0.8.47 踩的：看板列一多，最后几列被窗口边缘切掉、够不着。看板早就写着
`overflow-x: auto`，可它**从来没滚过** —— `.cm-scroller` 是 flex 容器，
`.cm-content` 这个项的宽度按**最大内容宽度**算，于是一排列把整条正文撑到比
窗口还宽。视图自己永远「装得下」，滚动条跑到了整个编辑器身上，横在页面最
底下：用户看到的是内容被切掉，而且不知道该滚哪里。

**`overflow-x: auto` 只决定「装不下时怎么办」，不决定「有多宽」。** 宽度是
上面给的，而上面正被这个盒子自己撑着 —— 这是个循环，光加 overflow 没用。

修法是让这个盒子的宽度**不由内容决定**：`contain: inline-size`。只含内联轴，
块级尺寸照旧由内容决定，CM 量 widget 高度那条路不受影响。

钉法：`board.scrollWidth > board.clientWidth`（自己真滚得动）**加上**
`scroller.scrollWidth <= scroller.clientWidth`（没把正文撑宽）。只钉前一条
测不出东西 —— 坏的那种状态里看板的盒子跟着变宽，它自己看来一切正常。
...
**M5b 已收官**（v0.6.1 起步，v0.6.38–40 收尾，2026-08-04 验收）：一个同步
按钮走完「提交 → 取 → 变基 → 推」；https + 令牌，令牌进钥匙串（安卓走
私有目录文件）；安卓经 rustls 传输层可同步；报错人话化；冲突就地解决
（ConflictView 逐段选边 → `sync_with` 带定稿重放）。**验收已过**：桌面 +
安卓真机对同一 GitHub 仓库双向往返不丢数据（作者实测）。冲突面板走的是
测试覆盖，真实冲突的实测还欠一次。
...
1. fine-grained 令牌的 **Contents 权限默认是 No access**，不改就连
   ...
   **Read and write**，且 Repository access 里勾上目标仓库（仓库晚于
   令牌创建时必然没勾）。
2. ~~同步失败时界面把 libgit2 原文直接甩出来~~ 已修（v0.6.39）：
   `sync.rs::humanize` 只翻 Http/Net 两类（401/403 → 令牌权限、404 →
   仓库地址、Net → 网络），**其余原样放行** —— 变基/检出的原文里带着
   文件名等线索，别包。安卓传输层（transport.rs）里有一份同口径的翻译。
...
单篇回退。**安卓移动端功能已经完成**：§5.5 之外的移动端体验已补齐（布局、手势、后台恢复时的全量 mtime 扫描等，见 DESIGN.md §八里程碑表）。
M6a 已作为 `v0.8.0` 发布；当前下一步仍是界面文案与细节收敛。
iOS 需要 Mac + Xcode，拆成 M6b（原计划 `v0.9.0`）并暂缓——不让它阻塞整体收敛。
