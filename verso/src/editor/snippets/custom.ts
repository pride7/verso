 ...
 *
 * 输入是一段 JSON 文本，格式与 Obsidian Latex Suite 相同。v0.6.26 之后日常编辑
 * 走设置里的表格，JSON 只作为内部存储和「从 Latex Suite 导入」的交换格式。
 *
 * ## 为什么错误信息要这么啰嗦
 *
 * 导入外部配置或手动改 settings.json 时仍可能写坏，而表现是「snippet 全都不响应」
 * —— 和「我记错了触发词」长得一模一样，极难自己诊断。
