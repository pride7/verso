...
/** `?raw` 拿源码文本。用 `import.meta.glob` 而不是 `fs` —— src 这个 tsconfig 里没有 node 的类型 */
...
const EXEMPT = ["./noGlobalDialog.test.ts", "./lib/dialog.ts"];
...
describe("确认框只走 lib/dialog", () => {
  ...
  it("没有 window.confirm / window.alert", () => {
    const bad = files
      ...
      .map(([path]) => path);
    expect(bad, "改成 `await confirm(...)`，从 lib/dialog 引入").toEqual([]);
  });
