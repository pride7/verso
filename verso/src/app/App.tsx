...
import { TemplatesView } from "../ui/TemplatesView";
import { HistoryView, type DiffSelection } from "../ui/HistoryView";
...
当前内容会先被记成一个版本，随时能再退回来。`);
  ...
  /**
   * 知识库骨架（§2.6）。三个视图分别解决不同的问题，共同构成完整总览：
   *
   * - **分类总览**：按分类分列的看板。分列的依据是分类而不是状态 —— 积累类的
   *   笔记没有进度，用 `status` 分列的话每一列都不动，看板将退化为占用空间较大的
   *   分组列表
   * - **最近更新**：按更新时间倒序，便于持续跟进近期内容
   * - **长期未更新**：筛出长期没有修改的内容，避免重要信息逐渐被遗忘。
   *   这一条依赖相对时间，写死日期会在下个月失效
   *
   * 同时将 `分类` 声明为单选：如果保留为自由文本，半年后可能同时出现「HPC」「hpc」
