...
| 用户自定义入口 | 目前只能改代码。`.folio/snippets.json` 的读取与设置界面等 **M4** |
...
**设置存在应用配置目录，不在 `.folio/` 里。** 设置是跟着人走的（字号、主题、自己写的 snippet），换一个 vault 不该重来一遍；而且 `.folio/` 按第 6 条铁律必须能整个删掉重建，放设置进去就违规了。文件是 `settings.json`，与 `recent.json` 同一目录。
