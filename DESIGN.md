...
这一块**只读**：写回 frontmatter 的路径是「解析成映射 → 重新序列化」（`write_note`），前端交回一段手改过的 YAML 没有对应的入口。要让源码模式里的 YAML 可编辑，得给 Rust 加一条「整篇原样写回」的命令，并让编辑器文档在源码模式下把 frontmatter 也含进来 —— 那会牵动保存、大纲行号、字数统计几处口径，值得单独做一次。
