...
改名时唯一需要向后兼容的是 ` ```folio-view ` 代码块 —— 那写在用户的 `.md` 里，是他们的数据不是我们的实现细节（§0 第 1 条）。解析器同时认新旧两种写法，`/` 菜单只插入新写法。`.folio/` 不需要迁移：它按定义就是可删除的派生数据。
...
| database 视图代码块 | ` ```verso-view ` | 见 §2.6。旧的 ` ```folio-view ` 仍然认，不认就等于弄坏用户已经写下的笔记 |
